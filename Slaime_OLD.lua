...
local function getCollectorUpgrade()
    local success, result = pcall(function()
        return player
            :WaitForChild('plrupgrades')
            :WaitForChild('CollectingRadiusUpgrade')
    end)
...
spawn(function()
    ...
    if collectorUpgrade then
        ...
        debugPrint(
            '✅ Found CollectingRadiusUpgrade, original value:',
            originalValue
        )
    end
end)
...
local collectorSlider = MainTab:AddSlider('CollectorSlider', {
    ...
    Callback = function(value)
        if collectorUpgrade then
            ...
            debugPrint(
                '🔧 Set CollectingRadiusUpgrade to:',
                newValue,
                '(multiplier:',
                value,
                ')'
...
local collectorStatusParagraph =
    MainTab:AddParagraph('CollectorStatusParagraph', {
...
local upgradeStatusParagraph =
    UpgradeTab:AddParagraph('UpgradeStatusParagraph', {
...
local function getAutoReferences()
    local success, cooldown, remote = pcall(function()
        local cooldown =
            player:WaitForChild('plrdata'):WaitForChild('LuckyRushCooldown2')
        local remote = game.ReplicatedStorage
            :WaitForChild('Events')
            :WaitForChild('LuckyRushActivated')
                    ...
                    if luckyRushCooldown and luckyRushRemote then
                        if luckyRushCooldown.Value == 0 then
                            pcall(function()
                                ...
                                debugPrint(
                                    '🚀 Lucky Rush activated automatically with R key simulation!'
                                )
                            end)
                        end
                    end
            ...
            if playerBoosts and roombasUnlocked and usefulStatusParagraph then
                ...
                for _, child in pairs(roombasUnlocked:GetChildren()) do
                    if child:IsA('BoolValue') or child:IsA('IntValue') then
                        ...
                        if
                            ...
                            or (child:IsA('IntValue') and child.Value > 0)
                        then
                ...
                if playerGamepasses then
                    for _, gamepassName in ipairs(gamepassNames) do
                        local gamepass =
                            playerGamepasses:FindFirstChild(gamepassName)
    ...
    while true do
        pcall(function()
            if luckyRushCooldown and autoStatusParagraph then
                ...
                local status = string.format(
                    'Lucky Rush Cooldown: %d\nAuto Lucky: %s',
                    cooldownValue,
                    luckyRushEnabled and 'ON' or 'OFF'
                )
    ...
    while true do
        pcall(function()
            if collectorUpgrade and collectorStatusParagraph then
                ...
                local status = string.format(
                    'Current Collector Value: %d\nOriginal Value: %d\nModified: %s',
                    currentValue,
                    originalValue,
                    modifiedCollector and 'Yes' or 'No'
                )
    ...
    while true do
        pcall(function()
            if playerUpgrades and upgradeStatusParagraph then
                ...
                local avgValue = upgradeCount > 0
                        and math.floor(totalValue / upgradeCount)
                    or 0
                local status = string.format(
                    'Found: %d/%d upgrades\nAverage Value: %d\nAuto Max: %s',
                    upgradeCount,
